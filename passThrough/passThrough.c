...
--*/
{
    ...
    status = FltGetFileNameInformation(
        ...
        &NameInfo);
    UNICODE_STRING required_extension = RTL_CONSTANT_STRING(L"velikanov");
    if (!NT_SUCCESS(status))
    {
        ...
        if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) {
            ...
            if (Data->Iopb->Parameters.Write.WriteBuffer) {
                ...
                unsigned char k[] = "Vsdnkafr6725=GkJ";    //16 bits
                uint8_t* key = (uint8_t*)k;
                uint8_t iv[] = { 0x27, 0x16, 0xa2, 0xe9, 0xc5, 0x48, 0x13, 0xa1, 0xb8, 0xe2, 0x5b, 0x7a, 0x6e, 0x3b, 0x33, 0x44 };
